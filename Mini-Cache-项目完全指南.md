# Mini-Cache 项目完全指南
> **定位**：Rust 高性能内存缓存服务器 + 前端监控面板
> **核心优势**：手写 TCP 异步服务 + 前端可视化 + 架构设计文档
...
1. **动态/静态资源分离**：前端静态页面与后端动态 API 分离部署
2. **协程模型**：Rust async/await 实现轻量级并发，对比 Go 协程
3. **缓存设计**：TTL 过期策略类比 Redis 的"惰性删除 + 定期删除"
4. **压力测试**：标准化压测流程，记录 QPS、P99 延迟、内存占用
5. **水平扩展思路**：未来可通过一致性哈希扩展为集群
...
- **阅读**：Ch 11（缓存设计），在文档中详细写 TTL 策略设计理由
- **刷题**：每天 1h 力扣（滑动窗口、堆、TopK）+ 0.5h 前端基础（React 原理、浏览器缓存）

### Week 4：压测 + 文档 + 优化 + 公开
- **TRPL 学习**：复习 Ch 4（Ownership）+ Ch 10（Lifetime）
- **项目目标**：
  - 用 `redis-benchmark` 或自写 client 压测，记录数据
  - 完善 `README.md`（放架构图、截图、演示链接）
  - 完善 `ARCHITECTURE.md`（完善架构设计文档）
  - 完善 `BENCHMARK.md`（压测方法论、结果、瓶颈分析）
  - GitHub 仓库从私有转为公开
- **阅读**：Ch 4（压力测试），产出标准化压测报告
- **刷题**：每天 1h 力扣（随机抽题）+ 1h 前端基础/文档写作
...
- [ ] ARCHITECTURE.md：包含架构设计概念（Ch 2, Ch 6, Ch 11, Ch 4）
- [ ] BENCHMARK.md：包含压测环境、方法、结果、瓶颈分析
- [ ] 前端：部署到 Vercel，提供在线演示链接
...
| 碎片时间 | 前端基础（React/Next.js/浏览器） | 0.5h |
